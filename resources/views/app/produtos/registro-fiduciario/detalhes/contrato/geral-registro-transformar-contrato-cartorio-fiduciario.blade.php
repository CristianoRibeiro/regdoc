<fieldset>
    ...
    <div class="row">
        <div class="col-12 col-md-6">
            ...
            <select name="id_estado_cartorio_ri" class="selecionar-cidade form-control selectpicker" title="Selecione">
        ...
        <div class="col-12 col-md-6">
            ...
            <select name="id_cidade_cartorio_ri" class="form-control selectpicker" title="Selecione" data-live-search="true" @if(count($cidades_disponiveis)<=0) disabled @endif>
    ...
    <div class="row mt-1">
        <div class="col-12">
            ...
            <select name="id_pessoa_cartorio_ri" class="form-control selectpicker" title="Selecione" data-live-search="true" @if(count($pessoas_cartorio_disponiveis)<=0) disabled @endif>
                @if(count($pessoas_cartorio_disponiveis)>0)
                    @foreach($pessoas_cartorio_disponiveis as $pessoa)
                        <option value="{{$pessoa->id_pessoa}}" @if($pessoa->id_pessoa == $id_pessoa_cartorio_ri) selected @endif>{{$pessoa->no_pessoa}}</option>
                    @endforeach
                @endif
            </select>
        </div>
