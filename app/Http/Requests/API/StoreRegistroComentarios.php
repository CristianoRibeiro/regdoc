...
class StoreRegistroComentarios extends FormRequest
{
    /**
    ...
    public function rules()
    {
        return [
            ...
            'arquivos.*.nome' => 'required_with:arquivos|max:255',
            'arquivos.*.bytes' => 'required_without:arquivos.*.url',
            'arquivos.*.url' => 'required_without:arquivos.*.bytes',
            'arquivos.*.extensao' => 'required_with:arquivos|max:10',
            'arquivos.*.mime_type' => 'required_with:arquivos|max:100',
            'arquivos.*.hash' => 'required_with:arquivos|max:32'
        ];
    }
